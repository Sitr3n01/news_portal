...
- **Formulários do Django admin (Unfold):** a mesma barra do editor do Wagtail, no lugar do cartão "Edição orientada", do cartão "Próximos passos" e da faixa de botões do rodapé. `templates/admin/change_form.html` (sobre o do Unfold) a inclui logo abaixo da topbar (`templates/admin/includes/form_bar.html`, dados de `apps/common/newsroom/admin_forms.py`), e ela fica presa ao rolar.
  ...
  - **Continua como era:** o aviso de sair com alterações (o `warn_unsaved_form` do Unfold), os erros de validação e as janelas popup de seleção de registro, que não têm a casca e mantêm o rodapé do Unfold. O calendário e o relógio dos campos de data abrem por cima da barra.
  - **Detalhes:** Ctrl+S envia uma vez só (segurar a tecla ou apertar de novo durante o envio não repete o POST). "Alterações não salvas" é anunciado pelo leitor de tela, vale também para campos de autocompletar e continua à vista depois de um erro de validação; a busca do seletor de grupos não conta. Quem só pode adicionar não vê o voltar para uma lista que não abriria (403), nem na barra nem na trilha da topbar. A barra é uma `<section>` com nome ("Barra do formulário"), e não um segundo `<header>`: a página continua com um só banner (o mesmo vale para o cabeçalho das listas).
...
- **Ações em massa do Django admin sem permissão declarada:** nenhuma ação própria (`@admin.action`) informa `permissions=`, então quem só pode ver uma tela também as vê e executa (por exemplo, "Arquivar mensagens"). É anterior à barra de seleção e fica para uma correção separada.
...
- **CSS sem uso:** as regras `.kb-model-guide*`, `.kb-form-next-*` e `.kb-form-step-list` de `static/admin/css/admin_ux.css` eram dos cartões que saíram das listas e dos formulários. Estão misturadas com as dos guias de operação, então ficaram para uma limpeza separada.
