...
- **Fontes e começo das entradas (14/09/2026).** O split só roda depois de as três webfonts carregarem: o módulo da página chama `document.fonts.load()` para Barlow Condensed, Inter e IBM Plex Mono, com teto de 2,5 s. Antes o módulo esperava só o `document.fonts.ready`, que resolvia antes de a página pedir as fontes. O texto era dividido com a fonte de fallback, e o `autoSplit` redividia os títulos quando a webfont chegava; em Cursos isso caía no meio da entrada do título, que saltava para o fim. O tempo do GSAP fica parado durante o split e volta a andar um quadro depois de a página aparecer, junto com o fade da [transição entre páginas](komuniki-transicao-e-rolagem.md). Sem o módulo da página, vale a regra anterior, descrita a seguir.
...
| `static/css/school-editorial.css` | Guard `.js [data-reveal]`, `.line-wrap`, `.line` e a correção dos acentos; folha em `?v=19` |
| `templates/base_school_editorial.html` | Classe `js` no `<head>`, os quatro scripts com `defer` (módulo em `?v=4`) e o `{% block footer %}` |
| `static/js/school-editorial.js` | Módulo da página: carrega as webfonts antes do split (`komunikiPage.fontsReady`) e avisa quando a página aparece (`komunikiPage.shown`) |
