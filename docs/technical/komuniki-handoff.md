...
- **Reveal de texto por linha:** GSAP 3.15.0 vendorizado (SplitText + ScrollTrigger), marcado no HTML com `data-reveal="mask"`, `data-reveal="fade"`, `data-reveal-group` e `data-reveal-hero`. Por enquanto só nos três exemplos aprovados da Home: título do hero, título do bloco do prêmio e títulos das trilhas. A aplicação nos demais títulos espera aprovação. Valores, correção dos acentos e validação em [Reveal de texto](komuniki-text-reveal.md).
...
- **Reveal de texto:** 36 verificações no Edge headless passaram: cascatas de 0,1 s e 0,2 s, reinício ao subir e descer, geometria idêntica ao texto estático em 1440 e 375 px, PT/EN, acessibilidade, movimento reduzido e Slow 4G sem flash. Também passaram o `collectstatic` com manifest, 468 testes e o Ruff. Detalhes e capturas em [Reveal de texto](komuniki-text-reveal.md#validação).
